...
class Dribble:
    ...
    def define_approach_orientation(self):
        ...
        self.approach_orientation = (
            a1 if a1_diff < a2_diff else a2
        )  # fixed normalized orientation
    ...
    def execute(self, reset, orientation, is_orientation_absolute, speed=1, stop=False):
        ...
        ) or np.linalg.norm(b_rel) > 0.4

        if reset:
            self.phase = 0
            if (
                behavior.previous_behavior == "Push_RL"
                and 0 < b_rel[0] < 0.25
                and abs(b_rel[1]) < 0.07
            ):
                self.phase = 1
                reset_dribble = True
        ...
        if self.phase == 0:  # walk to ball
            reset_walk = (
                reset
                and behavior.previous_behavior != "Walk"
                and behavior.previous_behavior != "Push_RL"
            ...
            if (
                reset or b_dist > 0.4
            ):  # stop defining orientation after getting near the ball to avoid noise
            ...
            if self.approach_orientation is not None:
                next_pos, next_ori, dist_to_final_target = (
                    self.path_manager.get_path_to_ball(
                        x_ori=self.approach_orientation,
                        x_dev=-0.24,
                        torso_ori=self.approach_orientation,
                        safety_margin=0.4,
                    )
                )
                ...
                if (
                    b_rel[0] < 0.26
                    and b_rel[0] > 0.18
                    and abs(b_rel[1]) < 0.04
                    and w.ball_is_visible
            ...
            # ------------------------ 2B. A better approach orientation is not needed but the robot cannot see the ball
                ...
                w.time_local_ms - w.ball_last_seen > 200
            ):  # walk to absolute target if ball was not seen
            ...
            # ------------------------ 2C. A better approach orientation is not needed and the robot can see the ball
                ...
                if (
                    ...
                    rel_target = b_rel + (
                        -0.23,
                        0,
                    )  # relative target is a circle (center: ball, radius:0.23m)
                    rel_ori = M.vector_angle(
                        b_rel
                    )  # relative orientation of ball, NOT the target!
                    dist = max(0.08, np.linalg.norm(rel_target) * 0.7)  # slow approach
                    behavior.execute_sub_behavior(
                        "Walk", reset_walk, rel_target, False, rel_ori, False, dist
                    )  # target, is_target_abs, ori, is_ori_abs, distance
        ...
        if self.phase == 1 and (
            stop or (b_dist > 0.5 and lost_ball)
        ):  # go back to walking
            self.phase += 1
            ...
            if orientation is None:
                ...
                self.env.dribble_rel_orientation = self.path_manager.get_dribble_path(
                    optional_2d_target=dribble_target
                )[1]
            elif is_orientation_absolute:
                self.env.dribble_rel_orientation = M.normalize_deg(
